...
export default function Page() {
  ...
  const DeleteToast = () => {
    return (showDeleteToast && <div
        id="toast-bottom-right"
        class="fixed flex items-center w-full max-w-xs space-x-4  divide-x divide-gray-200 shadow right-5 bottom-5 dark:text-gray-400 dark:divide-gray-700 space-x dark:bg-gray-800 p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800"
        role="alert"
      >
            ...
            {items.map((item) => (
              ...
              >
                ...
                <td class="px-6 py-4 flex items-center">
                  ...
                  <button class="font-medium text-red-600 dark:text-red-500 hover:underline ml-4" onClick={deleteItemHandler}>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
    ...
    return (
      ...
      >
        ...
        <ul className="inline-flex items-stretch -space-x-px">
         ...
         { Array.from({ length: totalPages }).map((_, index) => (<li key={index}>
            <button
             onClick={() => handleChangePage(index + 1)}
             disabled={currentPage === index + 1}
             type="button"
              className={`flex items-center justify-center text-sm py-2 px-3 leading-tight ${currentPage === index + 1 ? 'bg-indigo-600 text-white' :'bg-white text-gray-500' } border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white`}
            >
             {index + 1}
            </button>
          </li>))}
    ...
    <section className="p-2">
      <div className="mx-auto max-w-screen-xl px-2 lg:px-12">
        {/* <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
             {products.length ? <ItemsTable />:  <LoadingTable/> }
              <ItemsModal />
            </div>
          </div>
          <TableFooter />
        ...
        {products.length ? <FullItemsTable />:  <FullLoadingTable/> }
      </div>
    </section>
  );
}
